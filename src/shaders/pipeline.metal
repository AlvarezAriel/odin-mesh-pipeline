...
uint pushCube(
    ...
    // if(false) {
        ...
        if(has_left_neightbour) {
            tone += lightCalc(pos.xyz + float3(-1,0,0), camera_data, voxels_data);
            count += 1;
        }
        ...
        if(has_back_neightbour) {
            tone += lightCalc(pos.xyz + float3(0,0,-1), camera_data, voxels_data);
            count += 1;
        }
...
float4 fragmentMain(
    ...
    return float4(mix(input.tri.Color.rgb, float3(0.56, 0.32, 0.31), input.tri.Color.w), 1.0);
